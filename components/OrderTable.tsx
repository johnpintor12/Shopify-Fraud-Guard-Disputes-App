...
const OrderTable: React.FC<OrderTableProps> = ({
  ...
  // Reset page whenever tab or data changes
  useEffect(() => {
    setPage(1);
  }, [activeTab, orders]);
  ...
  const {
    ...
    return { riskCount: risk, openDisputesCount: open, historyCount: history, filteredOrders: filtered };
  }, [orders, activeTab]);
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      {/* Sub-tabs + top pagination bar */}
      <div className="flex items-center gap-4 mb-0 border-b border-zinc-200 bg-white px-4 pt-2 pb-2 rounded-t-lg">
        {/* Tabs */}
      ...
      <div className="flex-1 min-h-0 bg-white border border-zinc-200 rounded-b-lg rounded-t-none flex flex-col">
        <div className="flex-1 overflow-auto">
          {/* min-w-[1600px] forces horizontal scroll on laptop screens */}
              ...
              {visibleOrders.map((order) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 align-middle whitespace-nowrap text-zinc-700">
                    {order.customerName || (order as any).customer_email || 'Guest'}
                  </td>
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t border-zinc-200 text-xs text-zinc-600">
          ...
          <div className="text-[11px] text-zinc-500">
            50 rows per page (auto-limited for performance)
          </div>
        </div>
      </div>
    </div>
